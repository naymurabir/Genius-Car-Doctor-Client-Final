...
const Bookings = () => {
    ...
    useEffect(() => {
        if (user?.email) {
            axios.get(`https://genius-car-doctor-server-side.vercel.app/bookings?email=${user?.email}`, { withCredentials: true })
                .then(res => {
    ...
    const handleDeleteBooking = (id) => {
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    fetch(`https://genius-car-doctor-server-side.vercel.app/bookings/${id}`, {
                        method: 'DELETE'
                    })
                        .then(res => res.json())
                        .then(data => {
